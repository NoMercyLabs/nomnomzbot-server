...
public class RewardService : IRewardService
{
        ...
        CancellationToken cancellationToken = default)
    {
        if (!Guid.TryParse(rewardId, out var guid))
            return Result.Failure<RewardDetail>($"Invalid reward ID '{rewardId}'.", "VALIDATION_FAILED");

        var reward = await _db.Rewards
            .FirstOrDefaultAsync(r => r.Id == guid && r.BroadcasterId == broadcasterId, cancellationToken);

        if (reward is null)
            return Errors.NotFound<RewardDetail>("Reward", rewardId);

        if (request.Title is not null) reward.Title = request.Title;
        if (request.IsEnabled.HasValue) reward.IsEnabled = request.IsEnabled.Value;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return Result.Success(new PagedList<RewardListItem>(items, total, pagination.Page, pagination.PageSize));
    }
    ...
    public Task<Result> SyncWithTwitchAsync(string broadcasterId, CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(Result.Failure(
            "Twitch sync is not yet implemented.",
            "SERVICE_UNAVAILABLE"));
    }

    private static RewardDetail ToDetail(Reward r) => new(
        r.Id.ToString(),
        r.Title,
        r.Description,
        0,
