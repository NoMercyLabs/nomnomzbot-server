...
public sealed class SpotifyMusicProvider : IMusicProvider
{
    ...
    public async Task PlayAsync(string broadcasterId, CancellationToken cancellationToken = default)
    {
        ...
        await SendPlayerCommandAsync(HttpMethod.Put, $"{SpotifyApiBase}/me/player/play", token, null, cancellationToken);
    }
    ...
    public async Task PauseAsync(string broadcasterId, CancellationToken cancellationToken = default)
    {
        ...
        await SendPlayerCommandAsync(HttpMethod.Put, $"{SpotifyApiBase}/me/player/pause", token, null, cancellationToken);
    }
    ...
    public async Task SkipAsync(string broadcasterId, CancellationToken cancellationToken = default)
    {
        ...
        await SendPlayerCommandAsync(HttpMethod.Post, $"{SpotifyApiBase}/me/player/next", token, null, cancellationToken);
    }
    ...
    public async Task<TrackInfo?> GetCurrentTrackAsync(string broadcasterId, CancellationToken cancellationToken = default)
    {
        ...
        var response = await SendAsync(HttpMethod.Get, $"{SpotifyApiBase}/me/player/currently-playing", token, cancellationToken);
        if (response is null || response.StatusCode == HttpStatusCode.NoContent) return null;

        if (!response.IsSuccessStatusCode) return null;

        var json = await response.Content.ReadFromJsonAsync<SpotifyCurrentlyPlaying>(cancellationToken: cancellationToken);
        if (json?.Item is null) return null;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var url = $"{SpotifyApiBase}/search?q={Uri.EscapeDataString(query)}&type=track&limit={limit}";
        ...
        if (json?.Tracks?.Items is null) return [];
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var response = await SendPlayerCommandAsync(HttpMethod.Post, url, token, null, cancellationToken);
    ...
    private async Task<string?> GetTokenAsync(string broadcasterId, CancellationToken cancellationToken)
    {
        var service = await _db.Services
            .FirstOrDefaultAsync(s => s.BroadcasterId == broadcasterId
                && s.Name == ProviderName
                && s.Enabled
                && s.AccessToken != null, cancellationToken);

        if (service is null)
        {
            _logger.LogDebug("No Spotify service found for broadcaster {BroadcasterId}", broadcasterId);
            return null;
        }

        // Refresh if expiring within 5 minutes
        if (service.TokenExpiry.HasValue && service.TokenExpiry.Value <= DateTime.UtcNow.AddMinutes(5))
        {
        ...
        return service.AccessToken is not null
            ? _encryption.TryDecrypt(service.AccessToken)
            : null;
    }

    private async Task<string?> RefreshTokenAsync(Domain.Entities.Service service, CancellationToken cancellationToken)
    {
        if (service.RefreshToken is null) return null;
        ...
        var clientId = service.ClientId is not null ? _encryption.TryDecrypt(service.ClientId) : null;
        var clientSecret = service.ClientSecret is not null ? _encryption.TryDecrypt(service.ClientSecret) : null;

        if (clientId is null || clientSecret is null)
        {
            _logger.LogWarning("Spotify credentials not configured for broadcaster {BroadcasterId}", service.BroadcasterId);
            return null;
        }
        ...
        try
        {
            ...
            {
                _logger.LogWarning("Spotify token refresh failed for {BroadcasterId}: {Status}",
                    service.BroadcasterId, response.StatusCode);
                return null;
            }

            var json = await response.Content.ReadFromJsonAsync<SpotifyTokenResponse>(cancellationToken: cancellationToken);
            ...
            _logger.LogInformation("Refreshed Spotify token for {BroadcasterId}", service.BroadcasterId);
            return json.AccessToken;
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            _logger.LogError(ex, "Exception refreshing Spotify token for {BroadcasterId}", service.BroadcasterId);
            return null;
        }
    }
    ...
    private static TrackInfo MapToTrackInfo(SpotifyTrack track) => new()
    {
