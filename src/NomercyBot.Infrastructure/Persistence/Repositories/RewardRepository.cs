...
public class RewardRepository : GenericRepository<Reward>
{
    ...
    public Task<List<Reward>> GetByBroadcasterIdAsync(string broadcasterId, CancellationToken ct = default)
        => Set.Where(r => r.BroadcasterId == broadcasterId).OrderBy(r => r.Title).ToListAsync(ct);

    public Task<Reward?> GetByIdAndBroadcasterAsync(Guid id, string broadcasterId, CancellationToken ct = default)
        => Set.FirstOrDefaultAsync(r => r.Id == id && r.BroadcasterId == broadcasterId, ct);

    public Task<List<Reward>> GetPagedAsync(string broadcasterId, int page, int take, CancellationToken ct = default)
        => Set.Where(r => r.BroadcasterId == broadcasterId).OrderBy(r => r.Title)
