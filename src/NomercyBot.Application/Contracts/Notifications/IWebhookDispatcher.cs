...
public interface IWebhookDispatcher
{
    ...
    Task<WebhookDispatchResult> DispatchAsync(string webhookUrl, object payload, CancellationToken cancellationToken = default);

    /// <summary>Dispatch a payload with custom headers (e.g., HMAC signature).</summary>
    Task<WebhookDispatchResult> DispatchAsync(string webhookUrl, object payload, IDictionary<string, string> headers, CancellationToken cancellationToken = default);
}
